...
QUAN TRỌNG: 
1. Khi nhắc đến một thiết bị cụ thể, hãy LUÔN cung cấp một đường dẫn (Markdown link) tới trang chi tiết: `[Tên thiết bị](/giao-vien/thiet-bi/ID)`.
2. Nếu thiết bị có hình ảnh (URL ảnh được cung cấp trong dữ liệu), hãy LUÔN hiển thị ảnh đó bằng cú pháp Markdown: `![Tên thiết bị](URL_Ảnh)`.
   - LƯU Ý: URL ảnh có thể là đường dẫn tương đối (ví dụ: `/img/device/1.jpg`), hãy cứ dùng chính xác URL đó. Chatbot widget sẽ xử lý hiển thị.
3. Tuyệt đối KHÔNG trả lời "Thiết bị này không có ảnh" nếu bạn thấy có đường dẫn ảnh trong dữ liệu cung cấp.
4. Khuyến khích hiển thị cả ảnh và link cùng lúc để người dùng dễ quan sát.
5. LUÔN đối chiếu "Hạn trả dự kiến" của thiết bị với "THỜI GIAN HIỆN TẠI HỆ THỐNG" để xác định chính xác trạng thái quá hạn hay chưa.
Ví dụ: "Bạn có thể xem chi tiết tại [Máy chiếu Epson](/giao-vien/thiet-bi/12) ![Máy chiếu Epson](/img/device/epson.jpg)".
"""
...
# ============================================================
# BACKGROUND TASK: TỰ ĐỘNG THÔNG BÁO QUÁ HẠN
# ============================================================

def auto_check_overdue_task():
    """Vòng lặp chạy ngầm để kiểm tra quá hạn và gửi thông báo"""
    print("Background Thread: Overdue Check started.")
    SPRING_BOOT_API = "http://localhost:8080/api/ai-data"
    
    # Đợi server Spring Boot khởi động (nếu chạy cùng lúc)
    time.sleep(10)
    
    # Bộ nhớ tạm để tránh spam thông báo cho cùng một bản ghi quá hạn
    notified_records = set()
    
    while True:
        try:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] Checking for overdue borrowings...")
            
            # Gọi API lấy danh sách quá hạn
            response = requests.get(f"{SPRING_BOOT_API}/borrows/overdue", headers=INTERNAL_HEADERS, timeout=5)
            
            if response.status_code == 200:
                overdue_list = response.json()
                
                if overdue_list:
                    print(f"Found {len(overdue_list)} overdue borrowings.")
                    current_overdue_ids = {record.get('id') for record in overdue_list}
                    
                    # Xóa các ID không còn quá hạn khỏi bộ nhớ tạm
                    notified_records = {rid for rid in notified_records if rid in current_overdue_ids}
                    
                    for record in overdue_list:
                        record_id = record.get('id')
                        
                        # Nếu đã thông báo cho record này rồi thì bỏ qua
                        if record_id in notified_records:
                            continue
                            
                        print(f"DEBUG Record Data: {record}")
                        device_name = record.get('device_name', 'Thiết bị')
                        user_name = record.get('user_name', 'Giáo viên')
                        expected_return = record.get('expected_return', 'N/A')
                        
                        notification_msg = f"""
Kính gửi thầy/cô {user_name},
Thiết bị [{device_name}] quý thầy/cô mượn đã quá hạn trả ({expected_return}).
Vui lòng hoàn trả thiết bị cho bộ phận CSVC sớm nhất có thể.
"""
                        print(f"--- SENDING NOTIFICATION TO DB ---")
                        
                        # Gửi thong bao vào database thông qua Spring Boot API
                        try:
                            ...
                            if target_user_id:
                                notif_payload = {
                                    ...
                                    "title": "Cảnh báo quá hạn thiết bị",
                                    "content": notification_msg,
                                    "type": "OVERDUE"
                                }
                                requests.post(f"{SPRING_BOOT_API}/notifications", 
                                             headers=INTERNAL_HEADERS, 
                                             json=notif_payload, 
                                             timeout=5)
                                print(f"Successfully saved notification for user ID {target_user_id}")
                                notified_records.add(record_id)
                        except Exception as ne:
                            print(f"Failed to save notification to DB: {ne}")
                            
                        print(f"--- NOTIFICATION ALERT ---\n{notification_msg}-------------------------")
                else:
                    print("No overdue borrowings found.")
                    notified_records.clear()
            
            # Đợi một khoảng thời gian trước khi kiểm tra lại (ví dụ: 1 giờ thay vì mặc định)
            time.sleep(3600) # Mỗi giờ kiểm tra một lần để tránh spam
            
        except Exception as e:
            print(f"Error in overdue check task: {e}")
            time.sleep(60) # Đợi 1 phút nếu lỗi rồi thử lại
            
        # Kiểm tra mỗi giờ (3600 giây). Để demo/test bạn có thể giảm xuống 60s
        check_interval = int(os.getenv('OVERDUE_CHECK_INTERVAL', 3600))
        time.sleep(check_interval)

# Khởi chạy background thread
def start_background_tasks():
    thread = threading.Thread(target=auto_check_overdue_task, daemon=True)
    thread.start()
...
def chatbot():
    ...
    """
    try:
        # data = request.get_json() # Moved after imports
...
def build_data_context(real_data, user_role):
    ...
    # Lấy thời gian hiện tại để AI so sánh hạn trả
    now = datetime.now()
    current_time_str = now.strftime("%d/%m/%Y %H:%M")
    
    context_parts = [
        f"THỜI GIAN HIỆN TẠI HỆ THỐNG: {current_time_str}",
...
""")
    ...
    if 'maintenance_devices' in real_data and real_data['maintenance_devices']:
        devices_list = "\n".join([
            f"  {i+1}. [ID: {d['id']}] {d['name']} - Mã: {d['code']} - Phòng: {d.get('room', 'Chưa xác định')} - Loại: {d.get('category', 'N/A')} - [Ảnh: {d.get('hinh_anh','N/A')}]"
...
""")
    ...
    if 'recent_damages' in real_data and real_data['recent_damages']:
        damages_list = "\n".join([
            f"  {i+1}. {d['device_name']} [ID: {d.get('id','?')}]: {d['description'][:80]}... - Trạng thái: {d['status']} - Mức độ: {d.get('severity', 'N/A')} - [Ảnh: {d.get('device_image','N/A')}]"
...
""")
    ...
    if 'all_devices' in real_data and real_data['all_devices']:
        all_devices_list = "\n".join([
            f"  {i+1}. [ID: {d['id']}] {d['name']} - Mã: {d['code']} - Loại: {d.get('category', 'N/A')} - Phòng: {d.get('room', 'Chưa xác định')} - Trạng thái: {d.get('status', 'N/A')} - [Ảnh: {d.get('hinh_anh','N/A')}]"
...
""")
    ...
    if 'available_devices' in real_data and real_data['available_devices']:
        devices_list = "\n".join([
            f"  {i+1}. [ID: {d['id']}] {d['name']} - Mã: {d['code']} - Loại: {d.get('category', 'N/A')} - Phòng: {d.get('room', 'Chưa xác định')} - [Ảnh: {d.get('hinh_anh','N/A')}]"
...
""")
    ...
    if 'active_borrows' in real_data and real_data['active_borrows']:
        borrows_list = "\n".join([
            f"  {i+1}. {b.get('device_name','?')} [ID: {b.get('id','?')}] (Mã: {b.get('device_code','?')}) - [Ảnh: {b.get('device_image','N/A')}] - Người mượn: {b.get('user_name','N/A')} - Ngày mượn: {b.get('borrow_date','N/A')} - Trả dự kiến: {b.get('expected_return','N/A')}"
...
if __name__ == '__main__':
    ...
    ║  Notification Worker: ✓ Active (Background)               ║
    ║                                                           ║
    ║  Endpoints:                                               ║
    ║  - GET  /api/ai/health                                    ║
    ║  - POST /api/ai/chatbot                                   ║
    ...
    # Khởi chạy background task trước khi chạy Flask app
